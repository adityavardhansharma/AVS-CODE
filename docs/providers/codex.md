...
This guide is for people who want to use more than one Codex account in AVS Code.
...
- both accounts can see the same AVS Code/Codex sessions
...
In AVS Code Settings, name it something obvious:
...
In AVS Code Settings, add another Codex provider:
...
AVS Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, AVS Code treats it
