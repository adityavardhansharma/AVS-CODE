...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/AVS Code.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/AVS Code.app/Contents/Resources",
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("AVS Code (Alpha)")),
            readFileString: () =>
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/AVS Code (Alpha)");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["AVS Code (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "AVS Code (Alpha)");
